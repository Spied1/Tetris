...
//фикс цветов
//написать систему col для поворотов
...
std::vector<std::vector<int>> vec(22, std::vector<int>(12));
std::set<std::pair<float, float>> collision_cords;
//std::map<std::pair<float,float>,int> collors_cords;
std::vector<std::vector<int>> collors_cords(22, std::vector<int>(12));
...
public:
	...
	sf::Texture cubeT;
	sf::Sprite cubeS;
	cube(float TposX,float TposY,int color_id = 0):posX(TposX),posY(TposY),color_idk(color_id) {
		if (color_id == 0) {
...
void linexfixer(int t) {
	std::vector<std::vector<int>> tempvec(22, std::vector<int>(12));
	for (int i = 0; i != polesize.first; i++) {
		tempvec[i][0] = 1;
		tempvec[i][11] = 1;
...
int main() {
	float x = 96;
	float y = -32;
	int object_stage = abs(rand()) % 2;
	int obj_id = abs(rand()) % 7;
	int objR = 0;
	sf::Clock clock;
	float timer = 0;
	float delay = 0.5;
